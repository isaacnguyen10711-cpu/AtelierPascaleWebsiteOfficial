    ...
    <nav className="absolute left-0 top-0 z-50 w-full border-b border-white bg-transparent shadow-md">
      <div className={`mx-auto grid grid-cols-[25%_50%_25%] items-center px-4 py-4 font-medium text-sm ${isLoginPage ? 'text-black' : 'text-white'} md:text-base lg:text-lg`}>
        <div className="md:hidden">
          <Menu className={`${linkHover} h-4 w-4 md:h-6 md:w-6 lg:h-8 lg:w-8`} />
        </div>
        <div className="justify-self-center md:justify-self-start">
          <div className="flex flex-row gap-2 items-center">
          <Link className={linkHover} to="/">
            <House className="h-4 w-4 md:h-6 md:w-6 lg:h-8 lg:w-8 hidden md:flex" />
          </Link>
            <Link to="/" className={linkHover}>Atelier Pascale
            </Link>
          </div>
        </div>

        <div className="hidden justify-center md:gap-6 md:flex lg:gap-8">
